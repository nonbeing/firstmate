...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: a backend busy signature, or an actively running no-mistakes step for that crew's branch with a fresh active-step progress lease.
Pipeline ownership alone is not a turn-end lease, so a quiet active step without a live native agent pid or recent activity surfaces the bare turn-end even though the current state remains `working`.
