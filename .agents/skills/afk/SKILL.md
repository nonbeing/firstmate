...
- A message **without** the sentinel marker and **not** starting with `/afk` -> the captain is back.
  Run the existing `afk_exit` return path, which keeps the local identity-verified daemon for Codex ownership transfer and stops only that local daemon for every other primary.
